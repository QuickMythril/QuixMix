...
export function App() {
  ...
  return <div className="app-shell">
    <header className="app-header"><a className="brand" href="#" onClick={e=>{e.preventDefault();setView('listen');}}><span className="brand-icon">♪</span><span>Music<small>ON QORTIUM</small></span></a><nav aria-label="Main navigation"><button className={view==='listen'?'selected':''} onClick={()=>setView('listen')}>Listen</button><button className={view==='edit'?'selected':''} onClick={()=>setView('edit')}>Create playlist</button></nav><label className="theme-picker"><span>Theme</span><select aria-label="Theme" value={preference} onChange={event=>setPreference(event.target.value as ThemePreference)}><option value="system">System</option><option value="light">Light</option><option value="dark">Dark</option></select></label></header>
    <main>
